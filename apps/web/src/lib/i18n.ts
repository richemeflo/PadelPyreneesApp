"use client";

import i18n from "i18next";
import LanguageDetector from "i18next-browser-languagedetector";
import { initReactI18next } from "react-i18next";

const isServer = typeof window === "undefined";

const resources = {
  fr: {
    translation: {
      appName: "PadelPyrenees",
      nav: {
        home: "Accueil",
        ranking: "Classement",
        matchmaking: "Matchmaking",
        reservations: "Réservations",
        tournaments: "Tournois",
        profile: "Profil",
        login: "Connexion",
        register: "Inscription",
        signOut: "Se déconnecter",
      },
      auth: {
        signInTitle: "Connexion",
        signInSubtitle: "Connectez-vous avec votre email ou votre pseudo.",
        signInAction: "Se connecter",
        registerTitle: "Inscription",
        registerSubtitle: "Créez votre compte PadelPyrenees.",
        registerAction: "Créer mon compte",
        identifierLabel: "Email ou pseudo",
        usernameLabel: "Pseudo",
        emailLabel: "Email",
        passwordLabel: "Mot de passe",
        showPassword: "Afficher le mot de passe",
        hidePassword: "Masquer le mot de passe",
        noAccount: "Pas encore de compte ?",
        haveAccount: "Déjà un compte ?",
        registerLink: "Créer un compte",
        signInLink: "Se connecter",
        errors: {
          missingFields: "Veuillez remplir tous les champs.",
          invalidCredentials: "Identifiants invalides.",
          loginFailed: "Impossible de se connecter.",
          validationFailed: "Données invalides.",
          identifierRequired: "Veuillez saisir votre email ou pseudo.",
          emailInvalid: "Email invalide.",
          passwordTooShort: "Mot de passe trop court (min {{min}} caracteres).",
          usernameTooShort: "Pseudo trop court (min {{min}} caracteres).",
          identifierTaken: "Email ou pseudo deja utilise.",
          registerFailed: "Impossible de créer le compte.",
        },
      },
      profile: {
        title: "Mon profil",
        subtitle: "Mettez a jour vos informations et parametres.",
        infoTitle: "Informations",
        addressTitle: "Adresse",
        settingsTitle: "Parametres",
        saveAction: "Enregistrer",
        saving: "Enregistrement...",
        saveSuccess: "Profil mis a jour.",
        saveError: "Impossible d'enregistrer le profil.",
        missingUser: "Utilisateur introuvable.",
        invalidAddress: "Adresse invalide.",
        localePlaceholder: "Choisir une langue",
        fields: {
          pseudo: "Pseudo",
          email: "Email",
          locale: "Langue",
          streetNumber: "Numero",
          streetName: "Rue",
          city: "Ville",
          postalCode: "Code postal",
          country: "Pays (ISO)",
          formattedAddress: "Adresse formatee",
        },
        languages: {
          fr: "Francais",
          en: "Anglais",
        },
      },
      home: {
        info: "Mes informations",
        events: "Événements Padel Pyrénées",
        upcomingMatches: "Parties prévues",
        declareResults: "Déclarer les résultats",
        tournaments: "Tournois prévus",
        missions: "Missions",
        addResult: "Ajouter un résultat",
      },
      stats: {
        elo: "Elo",
        scheduledMatches: "Matchs prévus",
        scheduledTournaments: "Tournois prévus",
        resultsToDeclare: "Résultats à déclarer",
      },
      tournaments: {
        upcoming: "Tournois à venir",
        register: "S'inscrire",
        alreadyRegistered: "Inscrit",
      },
      tournamentsPage: {
        stats: {
          total: "Total tournois",
          openRegistration: "Inscriptions ouvertes",
          upcoming: "À venir",
          ongoing: "En cours",
        },
        myTournaments: {
          title: "Mes tournois",
          upcoming: "À venir",
          completed: "Terminés",
          emptyUpcoming: "Vous n'êtes inscrit à aucun tournoi à venir",
          emptyCompleted: "Vous n'êtes inscrit à aucun tournoi passé",
        },
        allTournaments: {
          title: "Tous les tournois",
          subtitle: "Découvrez et participez aux tournois de padel près de chez vous",
        },
        filters: {
          searchPlaceholder: "Rechercher un tournoi par nom, lieu...",
          typeLabel: "Type de tournoi",
          levelLabel: "Niveau",
          statusLabel: "Statut",
          sortLabel: "Trier par",
          clear: "Effacer les filtres",
          activeBadge: "Filtres actifs",
          activeType: "Type",
          activeLevel: "Niveau",
          activeStatus: "Statut",
          types: {
            all: "Tous les types",
          },
          levels: {
            all: "Tous les niveaux",
          },
          statuses: {
            all: "Tous les statuts",
          },
          sort: {
            date: "Date",
            title: "Titre",
            level: "Niveau",
            participants: "Participants",
            asc: "Asc",
            desc: "Desc",
          },
        },
        status: {
          registration: "Inscriptions ouvertes",
          upcoming: "À venir",
          ongoing: "En cours",
          completed: "Terminé",
        },
        levels: {
          all: "Tous",
          beginner: "Débutant",
          intermediate: "Intermédiaire",
          advanced: "Avancé",
          expert: "Expert",
        },
        types: {
          all: "Tous",
          singles: "Singles",
          doubles: "Doubles",
          mixed: "Mixte",
        },
        quickFilters: {
          beginner: "Débutants",
        },
        resultsCount: "{{count}} tournoi trouvé",
        resultsCount_other: "{{count}} tournois trouvés",
        noResultsTitle: "Aucun tournoi trouvé",
        noResultsSubtitle: "Aucun tournoi ne correspond à vos critères de recherche.",
        promotion: {
          title: "Organisez votre propre tournoi",
          description: "Créez et gérez facilement vos tournois de padel avec nos outils dédiés",
        },
        labels: {
          participants: "participants",
          registrations: "Inscriptions",
          deadline: "Limite",
          organizer: "Organisateur",
          info: "Informations",
          startDate: "Date de début",
          endDate: "Date de fin",
          location: "Lieu",
          spotsLeft: "{{count}} place restante",
          spotsLeft_other: "{{count}} places restantes",
          entryFee: "Inscription",
          prizes: "Prix",
          firstPlace: "1re place",
          secondPlace: "2e place",
          thirdPlace: "3e place",
        },
        actions: {
          details: "Détails",
          register: "S'inscrire",
          results: "Résultats",
          registerNow: "S'inscrire maintenant",
          full: "Complet",
          back: "Retour",
          share: "Partager",
          shareTitle: "Partager ce tournoi",
          shareCopy: "Copier le lien",
          shareWhatsapp: "WhatsApp",
          shareEmail: "Email",
          shareCopied: "Lien copié dans le presse-papiers",
          downloadPdf: "PDF",
          invite: "Inviter",
          inviteTitle: "Inviter un ami",
          inviteEmailPlaceholder: "Email de votre ami",
          inviteMessagePlaceholder: "Message personnalisé (optionnel)",
          sendInvite: "Envoyer l'invitation",
          inviteEmailRequired: "Veuillez saisir une adresse email",
          inviteSent: "Invitation envoyée à {{email}}",
          map: "Carte",
          organize: "Organiser un tournoi",
          resetFilters: "Réinitialiser les filtres",
          create: "Créer un tournoi",
          learnMore: "En savoir plus",
        },
        tabs: {
          location: "Localisation",
          bracket: "Tableau",
          results: "Résultats",
        },
        location: {
          title: "Localisation du tournoi",
          mapPlaceholder: "Carte interactive du club",
          coordinates: "Coordonnées",
        },
        bracket: {
          pendingTitle: "Tableau à venir",
          pendingSubtitle: "Le tableau sera généré après la clôture des inscriptions",
        },
        resultsSection: {
          title: "Résultats finaux",
          champion: "Champion",
          finalist: "Finaliste",
          thirdPlace: "3e place",
          pendingTitle: "Résultats à venir",
          pendingSubtitle: "Les résultats seront disponibles à la fin du tournoi",
        },
        toast: {
          registered: "Inscription au tournoi \"{{title}}\" confirmée !",
        },
        share: {
          text: "Découvrez ce tournoi de padel : {{title}}",
        },
      },
      ranking: {
        title: "Classement",
        mine: "Mon classement",
        categoryHeading: "Classement {{category}}",
        general: "Classement général",
        searchPlaceholder: "Rechercher un joueur...",
        filters: {
          genderAll: "Genre (tous)",
          genderMale: "Hommes",
          genderFemale: "Femmes",
          genderLabel: "Sexe",
          categoryAll: "Catégorie (toutes)",
          categoryLabel: "Catégorie",
          periodLabel: "Période",
          toggle: "Filtres",
          clear: "Effacer",
        },
        periods: {
          "7j": "7 jours",
          "1m": "1 mois",
          "3m": "3 mois",
          "6m": "6 mois",
          "1y": "1 an",
        },
        table: {
          rank: "Rang",
          player: "Joueur",
          elo: "Points ELO",
          trend: "Évolution",
          ratio: "Ratio V/D",
          category: "Catégorie",
        },
      },
      matchmaking: {
        proposals: "Propositions de match",
        accept: "Accepter",
        accepted: "Accepté",
        distance: "Distance",
        eloGap: "Écart Elo",
        schedule: "Créneau",
      },
      common: {
        loading: "Chargement...",
        error: "Une erreur est survenue",
        refresh: "Actualiser",
      },
    },
  },
  en: {
    translation: {
      appName: "PadelPyrenees",
      nav: {
        home: "Home",
        ranking: "Ranking",
        matchmaking: "Matchmaking",
        reservations: "Bookings",
        tournaments: "Tournaments",
        profile: "Profile",
        login: "Sign in",
        register: "Register",
        signOut: "Sign out",
      },
      auth: {
        signInTitle: "Sign in",
        signInSubtitle: "Sign in with your email or username.",
        signInAction: "Sign in",
        registerTitle: "Register",
        registerSubtitle: "Create your PadelPyrenees account.",
        registerAction: "Create account",
        identifierLabel: "Email or username",
        usernameLabel: "Username",
        emailLabel: "Email",
        passwordLabel: "Password",
        showPassword: "Show password",
        hidePassword: "Hide password",
        noAccount: "No account yet?",
        haveAccount: "Already have an account?",
        registerLink: "Create one",
        signInLink: "Sign in",
        errors: {
          missingFields: "Please fill in all fields.",
          invalidCredentials: "Invalid credentials.",
          loginFailed: "Unable to sign in.",
          validationFailed: "Invalid input.",
          identifierRequired: "Please enter your email or username.",
          emailInvalid: "Invalid email address.",
          passwordTooShort: "Password must be at least {{min}} characters.",
          usernameTooShort: "Username must be at least {{min}} characters.",
          identifierTaken: "Email or username already exists.",
          registerFailed: "Could not create account.",
        },
      },
      profile: {
        title: "Profile",
        subtitle: "Update your information and settings.",
        infoTitle: "Information",
        addressTitle: "Address",
        settingsTitle: "Settings",
        saveAction: "Save changes",
        saving: "Saving...",
        saveSuccess: "Profile updated.",
        saveError: "Unable to save profile.",
        missingUser: "User not found.",
        invalidAddress: "Invalid address.",
        localePlaceholder: "Select a language",
        fields: {
          pseudo: "Username",
          email: "Email",
          locale: "Language",
          streetNumber: "Street number",
          streetName: "Street name",
          city: "City",
          postalCode: "Postal code",
          country: "Country (ISO)",
          formattedAddress: "Formatted address",
        },
        languages: {
          fr: "French",
          en: "English",
        },
      },
      home: {
        info: "My information",
        events: "Padel Pyrénées events",
        upcomingMatches: "Upcoming matches",
        declareResults: "Declare results",
        tournaments: "Upcoming tournaments",
        missions: "Missions",
        addResult: "Add a result",
      },
      stats: {
        elo: "Elo",
        scheduledMatches: "Scheduled matches",
        scheduledTournaments: "Scheduled tournaments",
        resultsToDeclare: "Results to declare",
      },
      tournaments: {
        upcoming: "Upcoming tournaments",
        register: "Register",
        alreadyRegistered: "Registered",
      },
      tournamentsPage: {
        stats: {
          total: "Total tournaments",
          openRegistration: "Open registrations",
          upcoming: "Upcoming",
          ongoing: "Ongoing",
        },
        myTournaments: {
          title: "My tournaments",
          upcoming: "Upcoming",
          completed: "Completed",
          emptyUpcoming: "You are not registered for any upcoming tournaments",
          emptyCompleted: "You are not registered for any past tournaments",
        },
        allTournaments: {
          title: "All tournaments",
          subtitle: "Discover and join padel tournaments near you",
        },
        filters: {
          searchPlaceholder: "Search by name or location...",
          typeLabel: "Tournament type",
          levelLabel: "Level",
          statusLabel: "Status",
          sortLabel: "Sort by",
          clear: "Clear filters",
          activeBadge: "Active filters",
          activeType: "Type",
          activeLevel: "Level",
          activeStatus: "Status",
          types: {
            all: "All types",
          },
          levels: {
            all: "All levels",
          },
          statuses: {
            all: "All statuses",
          },
          sort: {
            date: "Date",
            title: "Title",
            level: "Level",
            participants: "Participants",
            asc: "Asc",
            desc: "Desc",
          },
        },
        status: {
          registration: "Open registration",
          upcoming: "Upcoming",
          ongoing: "Ongoing",
          completed: "Completed",
        },
        levels: {
          all: "All",
          beginner: "Beginner",
          intermediate: "Intermediate",
          advanced: "Advanced",
          expert: "Expert",
        },
        types: {
          all: "All",
          singles: "Singles",
          doubles: "Doubles",
          mixed: "Mixed",
        },
        quickFilters: {
          beginner: "Beginners",
        },
        resultsCount: "{{count}} tournament found",
        resultsCount_other: "{{count}} tournaments found",
        noResultsTitle: "No tournaments found",
        noResultsSubtitle: "No tournaments match your search criteria.",
        promotion: {
          title: "Host your own tournament",
          description: "Create and manage your padel tournaments with dedicated tools",
        },
        labels: {
          participants: "participants",
          registrations: "Registrations",
          deadline: "Deadline",
          organizer: "Organizer",
          info: "Information",
          startDate: "Start date",
          endDate: "End date",
          location: "Location",
          spotsLeft: "{{count}} spot left",
          spotsLeft_other: "{{count}} spots left",
          entryFee: "Entry fee",
          prizes: "Prizes",
          firstPlace: "1st place",
          secondPlace: "2nd place",
          thirdPlace: "3rd place",
        },
        actions: {
          details: "Details",
          register: "Register",
          results: "Results",
          registerNow: "Register now",
          full: "Full",
          back: "Back",
          share: "Share",
          shareTitle: "Share this tournament",
          shareCopy: "Copy link",
          shareWhatsapp: "WhatsApp",
          shareEmail: "Email",
          shareCopied: "Link copied to clipboard",
          downloadPdf: "PDF",
          invite: "Invite",
          inviteTitle: "Invite a friend",
          inviteEmailPlaceholder: "Friend's email",
          inviteMessagePlaceholder: "Custom message (optional)",
          sendInvite: "Send invite",
          inviteEmailRequired: "Please enter an email address",
          inviteSent: "Invite sent to {{email}}",
          map: "Map",
          organize: "Organize a tournament",
          resetFilters: "Reset filters",
          create: "Create tournament",
          learnMore: "Learn more",
        },
        tabs: {
          location: "Location",
          bracket: "Bracket",
          results: "Results",
        },
        location: {
          title: "Tournament location",
          mapPlaceholder: "Interactive club map",
          coordinates: "Coordinates",
        },
        bracket: {
          pendingTitle: "Bracket coming soon",
          pendingSubtitle: "The bracket will be generated after registrations close",
        },
        resultsSection: {
          title: "Final results",
          champion: "Champion",
          finalist: "Finalist",
          thirdPlace: "3rd place",
          pendingTitle: "Results coming soon",
          pendingSubtitle: "Results will be available at the end of the tournament",
        },
        toast: {
          registered: "Registration for \"{{title}}\" confirmed!",
        },
        share: {
          text: "Discover this padel tournament: {{title}}",
        },
      },
      ranking: {
        title: "Ranking",
        mine: "My ranking",
        categoryHeading: "{{category}} ranking",
        general: "Global ranking",
        searchPlaceholder: "Search for a player...",
        filters: {
          genderAll: "Gender (all)",
          genderMale: "Men",
          genderFemale: "Women",
          genderLabel: "Gender",
          categoryAll: "Category (all)",
          categoryLabel: "Category",
          periodLabel: "Period",
          toggle: "Filters",
          clear: "Clear",
        },
        periods: {
          "7j": "7 days",
          "1m": "1 month",
          "3m": "3 months",
          "6m": "6 months",
          "1y": "1 year",
        },
        table: {
          rank: "Rank",
          player: "Player",
          elo: "Elo points",
          trend: "Trend",
          ratio: "W/L ratio",
          category: "Category",
        },
      },
      matchmaking: {
        proposals: "Match proposals",
        accept: "Accept",
        accepted: "Accepted",
        distance: "Distance",
        eloGap: "Elo gap",
        schedule: "Slot",
      },
      common: {
        loading: "Loading...",
        error: "Something went wrong",
        refresh: "Refresh",
      },
    },
  },
};

if (!i18n.isInitialized) {
  if (!isServer) {
    i18n.use(LanguageDetector);
  }

  i18n
    .use(initReactI18next)
    .init({
      resources,
      fallbackLng: "fr",
      lng: "fr",
      interpolation: { escapeValue: false },
      detection: isServer
        ? undefined
        : {
            order: ["querystring", "localStorage", "navigator"],
            caches: ["localStorage"],
          },
    })
    .catch((error) => {
      console.error("i18n init failed", error);
    });
}

export { i18n };
